...
function refSampleIA16Bilinear5551_Persp(tex: Uint8Array, W: number, H: number, sF: number, tF: number, q: number, sm: 0|1|2, tm: 0|1|2): number {
...
describe('rspdl_ia16_tri_perspective_bilinear_reference_parity', () => {
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleIA16Bilinear5551_Persp(tex, W, H, sF, tF, q, sm, tm);
