...
// reference nearest sampling coordinate mapping
function nearest(s: number): number { return Math.round(s); }

// IA8: upper nibble = intensity (0..15), lower = alpha (0..15)
function to5(v: number, bits: number): number { const maxIn=(1<<bits)-1; return Math.round((v/maxIn)*31)&0x1f; }

function refSampleIA8Nearest5551(tex: Uint8Array, W: number, H: number, s: number, t: number, sm: 0|1|2, tm: 0|1|2): number {
  function map(i: number, n: number, m: 0|1|2): number { return m===1?wrap(i,n): m===2?mirror(i,n): clamp(i,n); }
  const ss = map(nearest(s), W, sm);
  const tt = map(nearest(t), H, tm);
...
describe('rspdl_ia8_tri_nearest_reference_parity (non-perspective)', () => {
  it('CLAMP/WRAP/MIRROR match software reference', () => {
    const width = 64, height = 48, origin = 0xB800;
    ...
    for (let i=0;i<modes.length;i++) {
      ...
      const uc: UcCmd[] = [
        ...
        { op: 'DrawIA8Tri', addr: texAddr, texW: W, texH: H,
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleIA8Nearest5551(tex, W, H, s, t, sm, tm);
