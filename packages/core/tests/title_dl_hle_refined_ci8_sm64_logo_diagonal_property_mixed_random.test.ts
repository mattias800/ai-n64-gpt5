...
// Deterministic property-like test: multiple mixed-sign diagonal shifts for composed SM64 row

describe('DL HLE composed SM64 row diagonal mixed-sign small shifts (property-like)', () => {
  it('maintains seam continuity and correct old/new pixel colors for mixed-sign ±1/±2 shifts', () => {
    const w=220,h=160, origin=0xD300;
    ...
    for (const c of cases) {
      ...
      // New seam for SIX at x+dx should be green (sample inside thickness at y+dy+2)
      expect(px(image, seamX(c.x + 76 + c.dx), c.y + c.dy + 2, w)).toEqual([0,255,0,255]);
      // Old seam side background appears on the side opposite horizontal motion
      ...
      // Old top bar background sampling depends on vertical motion
      const sampleY = c.dy > 0 ? c.y : c.y + 2;
      const [r1,g1,b1,a1] = px(image, seamX(c.x + 76), sampleY, w);
      expect(a1).toBe(255);
      expect(r1).toBeGreaterThan(200); expect(g1).toBeLessThan(50); expect(b1).toBeLessThan(50);
    }
  });
});
