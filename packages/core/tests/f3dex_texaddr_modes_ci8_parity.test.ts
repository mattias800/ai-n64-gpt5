...
// Build a G_SETTILE word pair with cms/cmt from small enums: 0=WRAP,1=MIRROR,2=CLAMP
function buildSETTILE(cms: number, cmt: number): [number, number] {
  // We'll pack cms into w1 bits 8..9 and cmt into w1 bits 18..19
  const op = 0xF5 << 24;
  const w0 = op >>> 0;
  const w1 = (((cmt & 3) << 18) | ((cms & 3) << 8)) >>> 0;
  return [w0, w1];
}

// Parity tests for CI8 addressing modes (NEAREST). We compare F3DEX path (G_SETTILE cms/cmt)
// against typed UC path (SetTexAddrMode).
describe('f3dex_texaddr_modes_ci8_parity', () => {
  it('wrap/mirror/clamp addressing parity (nearest)', () => {
    const width=128, height=96, origin=0x9000;
    const start=2, interval=3, frames=3, spOffset=1; // 3 frames: WRAP, MIRROR, CLAMP
    ...
    // TLUT with red at index 1; texture will be 1-filled.
    for (let i=0;i<256;i++) bus.storeU16(tlutAddr + i*2, i===1 ? COLORS_5551.red : 0);
    const W=16,H=16; for (let i=0;i<W*H;i++) bus.storeU8(texAddr+i, 1);

    // For each frame, draw a 16x16 sprite at x=60,y=10, but sample outside source
    // so addressing mode changes output. We'll shift s,t origin per frame.
    const modes = [0,1,2]; // WRAP,MIRROR,CLAMP
    for (let f=0; f<frames; f++) {
      let p = (dl0 + f*0x100)>>>0;
      // SETTIMG CI8
      const OP_SETTIMG = 0xFD<<24, SIZ_CI8 = 1<<19;
      ...
      // G_SETTILE modes (cms,cmt)
      const [w0,w1] = buildSETTILE(modes[f]!, modes[f]!);
      bus.storeU32(p, w0>>>0); p+=4; bus.storeU32(p, w1>>>0); p+=4;
      // TEXRECT to draw at (60,10)..(76,26)
      const OP_TEXRECT=0xE4<<24; const x=60,y=10;
    ...
    for (let f=0; f<frames; f++) {
      const modeEnc = modes[f]!; // 0 WRAP, 1 MIRROR, 2 CLAMP
      const sm = modeEnc===0?'WRAP':modeEnc===1?'MIRROR':'CLAMP';
      const tm = sm;
      const uc: UcCmd[] = [
        { op:'SetTLUT', tlutAddr, count:256 },
        { op:'SetCombine', mode:'TEXEL0' },
        { op:'SetTexAddrMode', sMode: sm as any, tMode: tm as any },
        { op:'DrawCI8', w:W, h:H, addr:texAddr, x:60, y:10 },
        { op:'End' },
      ];
      writeUcAsRspdl(bus2, (typedBase + f*0x100)>>>0, uc, stride);
    }
    const table2=(typedBase+0x1000)>>>0;
    for (let f=0; f<frames; f++) bus2.storeU32(table2 + f*4, (typedBase + f*0x100)>>>0);
