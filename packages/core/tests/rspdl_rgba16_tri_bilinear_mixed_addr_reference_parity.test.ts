...
function refSampleRGBA16Bilinear5551(tex: Uint16Array, W: number, H: number, s: number, t: number, sm: 0|1|2, tm: 0|1|2): number {
  ...
  function C(S:number,T:number){ return tex[T*W+S]??0; }
...
describe('rspdl_rgba16_tri_bilinear_mixed_addr_reference_parity', () => {
  it('non-perspective SM in {CLAMP,WRAP,MIRROR} x TM in {CLAMP,WRAP,MIRROR} match reference', () => {
    const width = 64, height = 48, origin = 0xD400;
    ...
    const W=8,H=8; const tex=new Uint16Array(W*H);
    for (let y=0;y<H;y++) for (let x=0;x<W;x++) {
      ...
      tex[y*W+x]=p; const off=(y*W+x)*2; bus.storeU8(texAddr+off,(p>>>8)&0xff); bus.storeU8(texAddr+off+1,p&0xff);
    }
    ...
    for (let fi=0; fi<frames; fi++) {
      const [sm, tm] = combos[fi]!; const frame = framesOut[fi]!;
      const minX = 6, maxX = 54, minY = 6, maxY = 40;
      ...
      for (let y = minY; y <= maxY; y += 2) for (let x = minX; x <= maxX; x += 2) {
        ...
        if (w0>=0&&w1>=0&&w2>=0) {
          ...
          const i16 = refSampleRGBA16Bilinear5551(tex, W, H, s, t, sm, tm);
