...
function makeCI8RedTex(w:number,h:number){
  const tex = new Uint8Array(w*h);
  tex.fill(1); // all index 1 = red
...
// Verify blending for CI8: blue background triangle then blended red CI8 tri -> overlap purple-ish
describe('rspdl_simple_blend_ci8_tri', () => {
  it('averages 5-bit channels when blending enabled (CI8)', () => {
    ...
    const { tex, tlut } = makeCI8RedTex(texW, texH);
    // Copy CI8 texture and TLUT into RDRAM
    const texAddr = dl + 0x2000; bus.rdram.bytes.set(tex, texAddr);
    const tlutAddr = dl + 0x4000;
