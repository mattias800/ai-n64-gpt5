...
function refSampleIA16Nearest5551(tex: Uint8Array, W: number, H: number, s: number, t: number, sm: 0|1|2, tm: 0|1|2): number {
  ...
  const p=(tt*W+ss)*2; const I=tex[p]??0; const A=tex[p+1]??0;
  const i5 = to5(I, 8); const a1 = A>=128?1:0;
  return (((i5&0x1f)<<11)|((i5&0x1f)<<6)|((i5&0x1f)<<1)|a1)>>>0;
}

describe('rspdl_ia16_tri_nearest_reference_parity (non-perspective)', () => {
  it('CLAMP/WRAP/MIRROR match software reference', () => {
    const width = 64, height = 48, origin = 0xBC00;
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleIA16Nearest5551(tex, W, H, s, t, sm, tm);
