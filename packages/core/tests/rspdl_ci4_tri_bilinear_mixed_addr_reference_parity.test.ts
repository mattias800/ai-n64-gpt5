...
function refSampleCI4BilinearColor5551(packed: Uint8Array, W: number, H: number, s: number, t: number, sm: 'CLAMP'|'WRAP'|'MIRROR', tm: 'CLAMP'|'WRAP'|'MIRROR', palOffset: number): number {
  ...
  function ci4At(s:number,t:number){ const idx=t*W+s; const byte=packed[idx>>1]??0; const hi=(byte>>>4)&0xF; const lo=byte&0xF; return (idx&1)===0?hi:lo; }
...
describe('rspdl_ci4_tri_bilinear_mixed_addr_reference_parity', () => {
  it('CI4 bilinear matches software reference for mixed S/T modes', () => {
    const width = 64, height = 48, origin = 0xCC00;
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleCI4BilinearColor5551(packed, W, H, s, t, sMode, tMode, (5&0xF)*16);
