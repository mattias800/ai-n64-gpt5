...
// Verify reversal in horizontal motion restores seam position and exposes background on the correct side.

describe('SM64 title sequence: horizontal reversal seam verification', () => {
  it('moves +1 in X then back -1 and verifies final seam and exposed side background', () => {
    const rdram = new RDRAM(1<<19);
    const bus = new Bus(rdram);
    const cpu = new CPU(bus);
    const sys = new System(cpu, bus);

    const w=220,h=140, origin=0xD420;
    ...
    const frames = [
      ...
      { at: 5, commands: drawRow(xS+1, y0, COLORS_5551.red) },
    ...
    // Final seam returns to x6; sample within top bar thickness
    expect(px(image, seamX(x6), y0+2, w)).toEqual([0,255,0,255]);

    // Since we moved left on the last frame, the area just to the right of the final seam should be background (white)
    const [r,g,b,a] = px(image, seamX(x6)+1, y0+8, w);
