...
// Perspective-correct CI8 bilinear sampling reference
...
function idxMode(i: number, n: number): number { return clamp(i, n); }
function foldFloat(u: number, n: number): number { const eps = 1e-7; return u < 0 ? 0 : u > (n - eps) ? (n - eps) : u; }
function refSampleCI8BilinearPersp(tex: Uint8Array, W: number, H: number, s: number, t: number): number {
  const su=foldFloat(s,W), tu=foldFloat(t,H);
  const s0=Math.floor(su), t0=Math.floor(tu);
  const s1=s0+1, t1=t0+1;
  const s0i=idxMode(s0,W), s1i=idxMode(s1,W);
  const t0i=idxMode(t0,H), t1i=idxMode(t1,H);
  const a=su-s0, b=tu-t0;
  const i00=tex[t0i*W+s0i]??0, i10=tex[t0i*W+s1i]??0, i01=tex[t1i*W+s0i]??0, i11=tex[t1i*W+s1i]??0;
  const i0=i00+(i10-i00)*a; const i1=i01+(i11-i01)*a; return Math.round(i0+(i1-i0)*b)>>>0;
}

describe('rspdl_ci8_tri_perspective_bilinear_reference_parity', () => {
  it('perspective CLAMP matches software reference', () => {
    const width = 64, height = 48, origin = 0xB000;
    ...
    // Compare a sparse grid inside triangle
    ...
    for (let y = minY; y <= maxY; y += 2) {
      for (let x = minX; x <= maxX; x += 2) {
        ...
        if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
          ...
          const idx = refSampleCI8BilinearPersp(tex, W, H, s, t);
