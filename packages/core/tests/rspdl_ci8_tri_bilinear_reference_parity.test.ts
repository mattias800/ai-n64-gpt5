...
// Software reference for CI8 bilinear sampling with address modes
...
function refSampleCI8BilinearColor5551(tex: Uint8Array, W: number, H: number, s: number, t: number, sm: 0|1|2, tm: 0|1|2): number {
  ...
  // TLUT maps idx -> 5-bit intensity = idx>>>3
...
// Assert per-pixel parity vs software reference for bilinear CI8 (non-perspective)
describe('rspdl_ci8_tri_bilinear_reference_parity', () => {
  it('non-perspective CLAMP/WRAP/MIRROR match software reference', () => {
    ...
    const out = res.frames[0]!; // first frame corresponds to dl0, etc.

    // Build software reference image for each frame and compare a sparse grid of pixels
    ...
    for (let fi=0; fi<frames; fi++) {
      ...
      // Sample a grid within the triangle bounding box
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleCI8BilinearColor5551(tex, W, H, s, t, sm, tm);
            const src = frame; // RGBA8888
            // Convert i16 to RGBA8888 like viScanout
            const r5=(i16>>>11)&0x1f, g5=(i16>>>6)&0x1f, b5=(i16>>>1)&0x1f, a1=i16&1;
            const r=(r5*255/31)|0, g=(g5*255/31)|0, b=(b5*255/31)|0, a=a1?255:0;
            const di=(y*width+x)*4;
            const gr=src[di], gg=src[di+1], gb=src[di+2], ga=src[di+3];
