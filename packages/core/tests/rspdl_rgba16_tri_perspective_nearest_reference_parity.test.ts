...
function refSampleRGBA16Nearest5551_Persp(tex: Uint16Array, W: number, H: number, sF: number, tF: number, q: number, sm: 0|1|2, tm: 0|1|2): number {
...
describe('rspdl_rgba16_tri_perspective_nearest_reference_parity', () => {
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleRGBA16Nearest5551_Persp(tex, W, H, sF, tF, l0*tri.q1 + l1*tri.q2 + l2*tri.q3, sm, tm);
