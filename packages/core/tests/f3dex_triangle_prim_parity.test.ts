...
// Parity test: mocked F3DEX VTX/TRI (0xB4/0xB5) under PRIM combine matches typed DrawPrimTri

describe('f3dex_triangle_prim_parity', () => {
  it('translator VTX/TRI produces same CRC as typed PRIM triangle', () => {
    ...
    // Write 3 vertices in DRAM: pack x (hi16), y (lo16) per 32-bit word
    function storeU32BE(addr:number, val:number){ busA.storeU32(addr, val>>>0); }
    storeU32BE(vtxRAM + 0*4, ((20<<16)|20)>>>0);
    storeU32BE(vtxRAM + 1*4, ((80<<16)|30)>>>0);
    storeU32BE(vtxRAM + 2*4, ((40<<16)|70)>>>0);

    // Build F3DEX DL in ROM:
    //  - set PRIM combine + color
    //  - VTX2D (0xB4) count-1 in low 12 bits of w0; w1=addr
    //  - TRI2D (0xB5) indices 0,1,2 in w0 low nibbles
    ...
    const opVTX = 0xB4<<24; const countminus1 = 2; W32(rom, p, (opVTX | countminus1)>>>0); p+=4; W32(rom, p, vtxRAM>>>0); p+=4;
    const opTRI = 0xB5<<24; const idxs = (0) | (1<<4) | (2<<8); W32(rom, p, (opTRI | idxs)>>>0); p+=4; W32(rom, p, 0); p+=4;
