...
// Verifies that PRIM combine mode fills with the prim color and matches between schedulers.
describe('rspdl_combine_prim_parity', () => {
  it('PRIM mode draws solid rect (ignores texture) and matches across schedulers', () => {
    ...
    const A = makeCtx();
    const baseA = (origin + fbBytes + 0xB000) >>> 0;
    const tlutA = baseA;
    const pixA = (baseA + 0x1000) >>> 0; // fill with varying indices
    ...
    for (let i = 0; i < frames; i++) {
      const f3d = [
        ...
        { op: 'G_SETTLUT' as const, addr: tlutA >>> 0, count: 256 }, // ensure TLUT set (not used by PRIM fill)
