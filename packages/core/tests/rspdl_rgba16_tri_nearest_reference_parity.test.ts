...
function refSampleRGBA16Nearest5551(tex: Uint16Array, W: number, H: number, s: number, t: number, sm: 0|1|2, tm: 0|1|2): number {
  ...
  const c = tex[tt*W+ss] ?? 0;
  return c >>> 0;
}

describe('rspdl_rgba16_tri_nearest_reference_parity (non-perspective)', () => {
  it('CLAMP/WRAP/MIRROR match software reference', () => {
    const width = 64, height = 48, origin = 0xC000;
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleRGBA16Nearest5551(tex, W, H, s, t, sm, tm);
