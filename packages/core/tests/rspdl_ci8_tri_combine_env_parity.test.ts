...
describe('rspdl_ci8_tri_combine_env_parity', () => {
  it('DrawCI8Tri under ENV combine matches DrawPrimTri with ENV color', () => {
    const width = 160, height = 120, origin = 0x6000;
    ...
    // ENV color = magenta
    const ENV = COLORS_5551.magenta;

    // TLUT and texture are irrelevant under ENV, but allocate
    for (let i = 0; i < 256; i++) bus.storeU16(tlutAddr + i * 2, 0x0000);
    for (let i = 0; i < 8 * 8; i++) bus.storeU8(texAddr + i, 0);

    // Frame 0: DrawCI8Tri with ENV combine
    const ucA: UcCmd[] = [
      { op: 'SetTLUT', tlutAddr, count: 256 },
      { op: 'SetEnvColor', color: ENV },
      { op: 'SetCombine', mode: 'ENV' },
      { op: 'DrawCI8Tri', addr: texAddr, texW: 8, texH: 8,
        x1: 50, y1: 30, s1: 0, t1: 0,
        x2: 90, y2: 30, s2: 7, t2: 0,
        x3: 50, y3: 70, s3: 0, t3: 7 },
      { op: 'End' },
    ];
    writeUcAsRspdl(bus, dlA, ucA, 128);

    // Frame 1: equivalent DrawPrimTri but using ENV color channel
    const ucB: UcCmd[] = [
      { op: 'SetEnvColor', color: ENV },
      { op: 'SetCombine', mode: 'ENV' },
      { op: 'DrawPrimTri', x1: 50, y1: 30, x2: 90, y2: 30, x3: 50, y3: 70 },
      { op: 'End' },
    ];
