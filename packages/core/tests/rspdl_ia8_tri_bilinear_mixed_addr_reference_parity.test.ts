...
function refSampleIA8Bilinear5551(tex: Uint8Array, W: number, H: number, s: number, t: number, sm: 0|1|2, tm: 0|1|2): number {
...
describe('rspdl_ia8_tri_bilinear_mixed_addr_reference_parity', () => {
  it('non-perspective SM in {CLAMP,WRAP,MIRROR} x TM in {CLAMP,WRAP,MIRROR} match reference', () => {
    const width = 64, height = 48, origin = 0xD000;
    ...
    for (let y=0;y<H;y++) for (let x=0;x<W;x++) {
      const i4 = Math.round(x*15/(W-1)) & 0xF; const a4 = Math.round(y*15/(H-1)) & 0xF;
      bus.storeU8(texAddr + y*W + x, ((i4<<4)|a4)&0xFF);
    }

    // Build DLs for 9 address mode combinations
    const dls: number[] = [];
    ...
    for (let fi=0; fi<frames; fi++) {
      ...
      for (let y = minY; y <= maxY; y += 2) for (let x = minX; x <= maxX; x += 2) {
        ...
        if (w0>=0&&w1>=0&&w2>=0) {
          ...
          const i16 = refSampleIA8Bilinear5551(tex, W, H, s, t, sm, tm);
