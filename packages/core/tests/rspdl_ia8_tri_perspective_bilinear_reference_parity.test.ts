...
function refSampleIA8Bilinear5551_Persp(tex: Uint8Array, W: number, H: number, sF: number, tF: number, q: number, sm: 0|1|2, tm: 0|1|2): number {
...
describe('rspdl_ia8_tri_perspective_bilinear_reference_parity', () => {
  it('perspective-correct CLAMP/WRAP/MIRROR match software reference', () => {
    const width = 64, height = 48, origin = 0xAC00;
    ...
    // Build a 16x16 IA8 texture: horizontal intensity ramp, vertical alpha ramp
    const W=16,H=16;
    for (let y=0;y<H;y++){
      for (let x=0;x<W;x++){
        const i4 = Math.round(x*15/(W-1)) & 0xF;
        const a4 = Math.round(y*15/(H-1)) & 0xF;
        const byte = ((i4<<4)|a4)&0xFF;
        bus.storeU8(texAddr + y*W + x, byte);
      }
    }
      ...
      for (let y = minY; y <= maxY; y += 2) {
        for (let x = minX; x <= maxX; x += 2) {
          ...
          if (w0 >= 0 && w1 >= 0 && w2 >= 0) {
            ...
            const i16 = refSampleIA8Bilinear5551_Persp(tex, W, H, sF, tF, q, sm, tm);
